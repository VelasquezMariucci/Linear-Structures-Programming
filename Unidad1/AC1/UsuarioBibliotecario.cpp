...
void UsuarioBibliotecario::verHistorialLibros(UsuarioCliente& u)
{
    u.getHistorialLibros();
}
...
void UsuarioBibliotecario::comprobarReservaCliente(UsuarioCliente& cliente)
{
    ...
    if (cliente.getReserva() == true)
    {
        ...
        std::cout << "El cliente reservo un libro" << std::endl;
    }
    else
    {
...
void UsuarioBibliotecario::devolverLibro(UsuarioCliente& cliente, Libro& libro)
{
    ...
    if (libro.getDisponibilidad() == 0 && cliente.getReserva() == 1)
    {
    ...
    {
        //muestra por consola que la devolucion no es posible y por que.
        std::cout << "Devolucion no posible" << std::endl;
        if (libro.getDisponibilidad() != 0)
        {
            std::cout << "El libro no estaba reservado." << std::endl;
        }

        if (!cliente.getReserva())
        {
            std::cout << "El cliente no tiene una reserva activa." << std::endl;
        }
    }
}
